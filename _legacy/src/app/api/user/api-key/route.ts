...
export async function POST(req: Request) {
  try {
    ...
    const newCharacterOcids = highLevelCharacters.map((char) => char.ocid);
    ...
    if (user?.mainCharacterId) {
      ...
      if (mainChar && !newCharacterOcids.includes(mainChar.ocid)) {
        clearMainCharacter = true;
      }
    }
    ...
    operations.push(
      prisma.user.update({
        ...
        data: {
          ...
          mainCharacterId: clearMainCharacter ? null : user?.mainCharacterId,
        },
        select: { id: true },
      })
    );
