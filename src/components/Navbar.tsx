...
export default function Navbar() {
  ...
  const activeLinkClasses =
    "block py-2 px-3 text-white bg-orange-500 rounded-sm md:bg-transparent md:text-orange-600 md:p-0";
  const inactiveLinkClasses =
    "block py-2 px-3 text-gray-900 rounded-sm hover:bg-gray-100 md:hover:bg-transparent md:hover:text-orange-600 md:p-0";
        ...
        <div className="flex items-center md:order-2 space-x-3 md:space-x-0 relative">
          {status === "unauthenticated" && (
            <Link href={"/login"}>
              <button
                type="button"
                className="text-white bg-orange-600 hover:bg-orange-700 focus:ring-4 focus:ring-orange-300 font-medium rounded-lg text-sm px-4 py-2 focus:outline-none cursor-pointer"
              >
                로그인
              </button>
            </Link>
          ...
          <button
            type="button"
            className="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 foucs:outline-none foucs:ring-2 focus:ring-gray-200"
